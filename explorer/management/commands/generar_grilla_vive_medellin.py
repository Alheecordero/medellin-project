"""
Genera grilla densa para Medellín + Envigado + zonas prioritarias de ViveMedellín.

No borra puntos existentes; evita duplicados por coordenada redondeada.
Pensado para alimentar scan_place_ids_gratis (Text Search IDs Only, gratis).

Zonas:
  - Medellín (16 comunas)
  - Envigado
  - El Poblado, Laureles/La 70, Centro, Envigado Centro (más denso)
...
from explorer.utils.vive_areas import VIVE_AREAS, VIVE_SEARCH_TYPES


class Command(BaseCommand):
    help = "Grilla Medellín + Envigado + zonas ViveMedellín (para scan gratis)."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default="",
            help="Solo un área (ej: 'El Poblado', 'Envigado'). Ver VIVE_AREAS.",
        )
    ...
    def handle(self, *args, **options):
        ...
        areas = VIVE_AREAS
        if solo:
            if solo not in areas:
                self.stdout.write(self.style.ERROR(f"Área desconocida: {solo}"))
                self.stdout.write(f"Opciones: {', '.join(areas.keys())}")
                return
            areas = {solo: areas[solo]}
        ...
        self.stdout.write(f"\n{'═' * 60}")
        self.stdout.write(self.style.SUCCESS("📊 GRILLA VIVE MEDELLÍN"))
        ...
        if dry_run:
            ...
            self.stdout.write("   → python manage.py scan_place_ids_gratis --dry-run")
            self.stdout.write("   → python manage.py scan_place_ids_gratis\n")
